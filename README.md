...
Landing Zone — DBFS /landing (JSON bruto)
    ...
    ▼
Schema estruturado + filtro de registros inválidos
    │
    ▼
Transformações: cast de tipos, spread_24h, trend_24h
    │
    ├── writeStream → Delta Lake /output
    ├── groupBy + agg → Delta Lake /aggregations
    └── window() sliding 60min + tumbling 30min → /windows
```
...
|----------|---------|--------|
| Enunciado base (ingestão + ETL + Parquet) | `stream_pipeline_coingecko.ipynb` Parte 4–6 | ✅ |
| Bônus 1 — Schema e validação | Parte 3 + filtros na Parte 6 | ✅ |
| Bônus 2 — Agregação | Parte 8 | ✅ |
| Bônus 3 — Window Functions | CéluPartela 9 (sliding + tumbling) | ✅ |
...
4. A Parte 5 gera os dados (aguarda ~75s para 5 batches)
5. Verificar outputs em `/Volumes/workspace/default/crypto_pipeline"`
